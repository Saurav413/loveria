    ...
    script.onload = () => {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async (response: { credential: string }) => {
          ...
          if (!start.ok) {
            setStatus(start.data.error || "Could not start Google sign-up.");
            return;
          }
  ...
  return (
    <main className="page-shell" style={{ maxWidth: 460, paddingTop: "4rem" }}>
      <div className="glass-card">
        <h1 className="brand" style={{ cursor: "pointer", marginTop: 0 }} onClick={() => router.push("/login")}>
          Loveria
        </h1>
        <p className="muted">Sign up to start your shared journey.</p>
        ...
        <button
          ...
          onClick={() => alert("Apple signup will be connected in a future update.")}
        >
          Sign up with Apple
        </button>
